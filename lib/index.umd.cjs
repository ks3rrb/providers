(function(global2, factory) {
  typeof exports === "object" && typeof module !== "undefined" ? factory(exports, require("iso-639-1"), require("cheerio"), require("crypto-js"), require("unpacker"), require("nanoid"), require("form-data"), require("cookie"), require("set-cookie-parser")) : typeof define === "function" && define.amd ? define(["exports", "iso-639-1", "cheerio", "crypto-js", "unpacker", "nanoid", "form-data", "cookie", "set-cookie-parser"], factory) : (global2 = typeof globalThis !== "undefined" ? globalThis : global2 || self, factory(global2.index = {}, global2["iso-639-1"], global2.cheerio, global2["crypto-js"], global2.unpacker, global2.nanoid, global2["form-data"], global2.cookie, global2["set-cookie-parser"]));
})(this, function(exports2, ISO6391, cheerio, CryptoJS, unpacker, nanoid, FormData, cookie, setCookieParser) {
  "use strict";
  ...
  async function addOpenSubtitlesCaptions(captions, ops, media) {
    try {
      ...
      const apiUrl2 = `https://subs.whvx.net/search?id=${String(imdbId)}${season && episode ? `&season=${season}&episode=${episode}` : ""}`;
      const rawResponse = await ops.proxiedFetcher(apiUrl2, {
        headers: {
  ...
  const doodScraper = makeEmbed({
    ...
    async scrape(ctx) {
      ...
      ctx.progress(50);
      if (((_a = vidScrape.videoUrl) == null ? void 0 : _a.length) === 0) {
  ...
  const baseUrl$c = "https://api.whvx.net";
  const headers$3 = {
  ...
  async function comboScraper$d(ctx) {
    ...
    const res = await ctx.fetcher(`${baseUrl$c}/status`, {
      headers: headers$3
    });
  ...
  const novaScraper$1 = makeEmbed({
    ...
    async scrape(ctx) {
      ...
      try {
        const search2 = await ctx.fetcher(`${baseUrl$c}/search?query=${encodeURIComponent(ctx.url)}`);
        if (search2.statusCode === 404) {
          throw new Error("No files found");
        }
        const result = await ctx.fetcher(`${baseUrl$c}/source?resourceId=${encodeURIComponent(search2.url)}`);
  ...
  const baseUrl$b = "https://nsbx.wafflehacker.io";
  const headers$2 = {
    Origin: "https://www.vidbinge.com",
    Referer: "https://www.vidbinge.com"
  };
  async function comboScraper$c(ctx) {
    ...
    const res = await ctx.fetcher(`${baseUrl$b}/status`, { headers: headers$2 });
    if (((_a = res.providers) == null ? void 0 : _a.length) === 0) {
      throw new NotFoundError("No providers available");
    }
    const embeds = res.providers.map((provider) => {
  ...
  const [deltaScraper, alphaScraper] = providers$4.map(embed$4);
  const { AES: AES$1, MD5 } = CryptoJS;
  function mahoaData(input, key2) {
    const a = AES$1.encrypt(input, key2).toString();
  ...
  function embed$3(provider) {
    return makeEmbed({
      ...
      async scrape(ctx) {
        ...
        try {
          const token = window["vbtk"];
          let searchUrl = `${baseUrl$c}/search?query=${encodeURIComponent(ctx.url)}&provider=${provider.id}`;
          if (token) {
            searchUrl += `&token=${encodeURIComponent(token)}`;
          }
          const search2 = await ctx.fetcher.full(searchUrl, { headers });
          if (search2.statusCode === 429) {
            throw new Error("Rate limited");
          } else if (search2.statusCode !== 200) {
            throw new NotFoundError("Failed to search");
          ...
          const result = await ctx.fetcher(
            `${baseUrl$c}/source?resourceId=${encodeURIComponent(search2.body.url)}&provider=${provider.id}`,
            { headers }
          );
          clearInterval(interval);
          ctx.progress(100);
  ...
  const [novaScraper, astraScraper, orionScraper] = providers$3.map(embed$3);
  ...
  const universalScraper$8 = async (ctx) => {
    ...
    const headersString = JSON.stringify(headers2);
  ...
  const baseUrl$a = "https://soaper.live";
  const universalScraper$7 = async (ctx) => {
    ...
    if (ctx.media.type === "show") {
      ...
      const showPage = await ctx.proxiedFetcher(showLink, { baseUrl: baseUrl$a });
    ...
    if (!showLink) throw new NotFoundError("Content not found");
    const contentPage = await ctx.proxiedFetcher(showLink, { baseUrl: baseUrl$a });
    ...
    const streamRes = await ctx.proxiedFetcher(infoEndpoint, {
      ...
      body: formData,
      headers: {
        referer: `${baseUrl$a}${showLink}`
      }
    });
    const streamResJson = JSON.parse(streamRes);
    const playlistUrl = `${baseUrl$a}${streamResJson.val}`;
    const backupPlaylistUrl = `${baseUrl$a}${streamResJson.val_bak}`;
    const backupPlayRes = await ctx.proxiedFetcher(backupPlaylistUrl, { headers: { referer: `${baseUrl$a}${showLink}` } });
    const playlistRes = await ctx.proxiedFetcher(playlistUrl, { headers: { referer: `${baseUrl$a}${showLink}` } });
    ...
    if (Array.isArray(streamResJson.subs)) {
      for (const sub of streamResJson.subs) {
        ...
        const fullSubUrl = `${proxyPrefix}${encodeURIComponent(baseUrl$a + sub.path)}`;
        let language = "";
  ...
  const universalScraper$6 = async (ctx) => {
    const apiRes = await ctx.fetcher(
    ...
    const captions = (subtitles == null ? void 0 : subtitles.map((sub) => {
      const language = labelToLanguageCode(sub.lang) || "und";
      const captionType = getCaptionTypeFromUrl(sub.file) || "vtt";
      return {
        id: sub.file,
        url: sub.file,
        type: captionType,
  ...
  const evalCodeRegex$4 = /eval\((.*)\)/g;
  const fileRegex$3 = /file:"(.*?)"/g;
  ...
  const packedRegex$2 = /<script type=(?:['"]text\/javascript['"])?\s*>(eval\(function\(p,a,c,k,e,[\s\S]*?\))\s*<\/script>/;
  ...
  const filelionsScraper = makeEmbed({
    ...
    async scrape(ctx) {
      ...
      const linkMatch = unpacked.match(linkRegex$4);
  ...
  const streambucketScraper = makeEmbed({
    ...
    async scrape(ctx) {
      ...
      regexResult = decoded.match(linkRegex$2);
      if (!regexResult) {
  ...
  const vTubeScraper = makeEmbed({
    ...
    scrape: async (ctx) => {
      ...
      const evalCode = html("script").text().match(evalCodeRegex$3);
      if (!evalCode) throw new Error("Failed to find eval code");
  ...
  const twoEmbedScraper = makeSourcerer({
    id: "2embed",
    name: "2Embed",
    rank: 140,
    disabled: false,
  ...
  const filmxyScraper = makeSourcerer({
    id: "filmxy",
    name: "Filmxy",
    rank: 111,
    disabled: true,
  ...
  const linkRegex$1 = /'hls':\s*'([a-zA-Z0-9+/]+={0,2})'/;
  ...
  const voeScraper = makeEmbed({
    ...
    rank: 180,
    async scrape(ctx) {
      let embedRes = await ctx.proxiedFetcher.full(ctx.url);
      let embed2 = embedRes.body;
      const redirectMatch = embed2.match(redirectRegex);
      if (redirectMatch) {
        embedRes = await ctx.proxiedFetcher.full(redirectMatch[1]);
        embed2 = embedRes.body;
      }
      ...
      const streamUrl = playerSrc[1];
      const decodedStreamUrl = safeBase64Decode(streamUrl);
  ...
  const baseUrl$9 = "https://lmscript.xyz";
  ...
  const baseUrl$8 = "https://tugaflix.best/";
  ...
  const baseUrl$7 = "https://mirrors.whvx.net/";
  async function comboScraper$7(ctx) {
    ...
    const embeds = data2.providers.map((provider) => {
      return {
        embedId: provider,
        url: JSON.stringify(query)
      };
  ...
  const mirrorsScraper = makeSourcerer({
    id: "whvxMirrors",
    name: "WHVX Mirrors",
    rank: 158,
    flags: [flags.CORS_ALLOWED],
...
%3A%20%22https%3A%2F%2Fcloseload.top%22%7D`;
  ...
  const fileMoonScraper = makeEmbed({
    ...
    scrape: async (ctx) => {
      ...
      const evalCode = embedHtml("script").text().match(evalCodeRegex$1);
      if (!evalCode) throw new Error("Failed to find eval code");
      const unpacked = unpacker.unpack(evalCode[0]);
  ...
  const [mp4hydraServer1Scraper, mp4hydraServer2Scraper] = providers$1.map(embed$1);
  function decode(str) {
  ...
  const streamwishScraper = makeEmbed({
    ...
    scrape: async (ctx) => {
      ...
      const playlistUrl = file == null ? void 0 : file[1];
  ...
  const turbovidScraper = makeEmbed({
    ...
    async scrape(ctx) {
      ...
      const proxiedPlaylist = `https://m3u8.justchill.workers.dev/?url=${encodeURIComponent(playlist)}&origin=https://turbovid.eu&referer=https://turbovid.eu/`;
      return {
        stream: [
          {
            type: "hls",
  ...
  const vidkerScraper = makeEmbed({
    ...
    async scrape(ctx) {
      var _a, _b;
      const embed2 = await ctx.proxiedFetcher(ctx.url);
      const sourcesMatch = (_a = embed2.match(/sources:\s*(\[.*?\])/)) == null ? void 0 : _a[1];
      const tracksMatch = (_b = embed2.match(/tracks:\s*(\[[^\]]*\])/)) == null ? void 0 : _b[1];
      if (!sourcesMatch) throw new Error("No sources found");
      const sources = JSON.parse(sourcesMatch.replace(/(\w+):"(\S*?)"/g, '"$1":"$2"'));
      const tracks = JSON.parse(
        (tracksMatch == null ? void 0 : tracksMatch.replace(/\s+/g, "").replace(/(\w+):"(\S*?)"/g, '"$1":"$2"')) ?? "[]"
      );
  ...
  const vidMolyScraper = makeEmbed({
    id: "vidmoly",
    name: "Vidmoly",
    rank: 194,
    async scrape(ctx) {
    ...
    return makeEmbed({
      ...
      async scrape(ctx) {
        ...
        try {
          ...
          const urlEncodedTitle = encodeURIComponent(query.title || "");
          const urlEncodedReleaseYear = encodeURIComponent(query.releaseYear || "");
          let url = `${baseUrl$7}scrape?title=${urlEncodedTitle}&type=${query.type}&releaseYear=${urlEncodedReleaseYear}&provider=${provider.id}`;
  ...
  const [amznScraper, ntflxScraper] = providers.map(embed);
  ...
  const baseUrl$6 = "https://hindiscrape.whvx.net";
  async function comboScraper$6(ctx) {
    ...
    const fileData = playerPage.sources;
    const embeds = [];
    for (const stream of fileData) {
      console.log(stream);
      const url = stream.file;
      if (!stream.file) {
        console.log(`Skipping stream due to missing file:`, stream);
        continue;
      }
  ...
  const hindiScraper = makeSourcerer({
    id: "hindiscraper",
    name: "Jalebi Scraper",
    rank: 10,
    flags: [flags.CORS_ALLOWED],
  ...
  const baseUrl$5 = "https://catflix.su";
  ...
  const baseUrl$4 = "https://www.goojara.to";
  ...
  let baseUrl$3 = "https://m4ufree.tv";
  const universalScraper$2 = async (ctx) => {
    var _a, _b, _c;
    const homePage = await ctx.proxiedFetcher.full(baseUrl$3);
    baseUrl$3 = new URL(homePage.finalUrl).origin;
  ...
  const baseUrl$2 = "https://arbplus.net";
  async function comboScraper$3(ctx) {
    ...
    search$(".item-box").each((_, element) => {
      var _a2;
      const [, title, year] = ((_a2 = search$(element).find("a").attr("title")) == null ? void 0 : _a2.match(/^(.*?)\s*(?:\(?\s*(\d{4})(?:\s*-\s*\d{0,4})?\s*\)?)?\s*$/)) || [];
      const id2 = search$(element).find("span").first().attr("data-itemid");
      if (!title || !id2) return;
      searchResults.push({ title, year: year ? Number(year) : void 0, id: id2 });
    });
    const id = (_a = searchResults.find((x) => x && compareMedia(ctx.media, x.title, x.year))) == null ? void 0 : _a.id;
    if (!id) throw new NotFoundError("No watchable item found");
    const apiRes = await ctx.proxiedFetcher("/wp-admin/admin-ajax.php", {
      baseUrl: baseUrl$2,
      method: "POST",
      body: new URLSearchParams({
    ...
    const iframeUrl = cheerio.load(apiRes.embed_url)("iframe").attr("src");
    if (!iframeUrl) throw new Error("iFrame Url not found");
    const iframePage$ = cheerio.load(await ctx.proxiedFetcher(iframeUrl));
    const links = [];
    iframePage$(".dropdown-item").each((_, element) => {
      const link = iframePage$(element).attr("href");
      if (link == null ? void 0 : link.startsWith("https:")) links.push(link);
    });
    const embeds = [];
    for (const url of links) {
      if (links.includes(url)) {
        const origin2 = new URL(url).hostname;
        let embedId;
        switch (origin2) {
          case "vidker.com":
            embedId = "vidker";
  ...
  const moviplusScraper = makeSourcerer({
    id: "moviplus",
    name: "MoviPlus",
    rank: 92,
    disabled: true,
  ...
  const baseUrl$1 = "https://mp4hydra.org/";
  ...
  const netMirrorScraper = makeSourcerer({
    id: "netmirror",
    name: "NTFLX",
    rank: 130,
    disabled: true,
